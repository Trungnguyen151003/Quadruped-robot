...
from Robot import test_walking
import random
from collections import deque
import pybullet
from Robot import bullet_client
...
class SpotEnv(gym.Env):
    ...
    def hard_reset(self):
        ...
        model_path = "Robot/Spot1504/urdf/Spot1504.urdf"
    ...
    # # Tính chiều cao mong muốn theo mặt phẳng hỗ trợ (đã có sẵn trong code gốc)
    #     desired_height = (robot_height_from_support_plane / np.cos(wedge_angle) +
    #                       np.tan(wedge_angle) * (pos[0] * np.cos(self.incline_ori) + 0.5))

    # # --- 1. Chuẩn hóa sai lệch (error) ---
    #     roll_error = (rpy[0] - self.support_plane_estimated_roll) / np.pi
    #     pitch_error = (rpy[1] - self.support_plane_estimated_pitch) / np.pi
    #     yaw_error = rpy[2] / np.pi
    #     height_error = (desired_height - current_height) / desired_height

    # # --- 2. Reward ổn định ---
    #     roll_reward = np.exp(-10 * roll_error ** 2)
    #     pitch_reward = np.exp(-10 * pitch_error ** 2)
    #     yaw_reward = np.exp(-15 * yaw_error ** 2)
    #     height_reward = np.exp(-20 * height_error ** 2)
    ...
    #     forward_reward = 10 * step_distance_x  # scale vừa phải, tránh reward quá lớn
    ...
    #         reward = fall_penalty
    #     else:
    #         reward = (roll_reward + pitch_reward + yaw_reward + height_reward + forward_reward)

    # # --- 5. Penalize nếu đứng yên quá lâu ---
    #     self.step_disp.append(step_distance_x)
    #     if self._n_steps > 150 and sum(self.step_disp) < 0.05:
    #         reward -= standing_penalty

    # # --- 6. Clip reward để ổn định gradient ---
    ...
    def _get_reward(self):
        ...
        pitch_reward = np.exp(-10 * pitch_error ** 2)
        yaw_reward = np.exp(-15 * yaw_error ** 2)
        height_reward = np.exp(-800 * height_error ** 2)
